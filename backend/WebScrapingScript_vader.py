...
from vaderSentiment.vaderSentiment import SentimentIntensityAnalyzer  # VADER added

# Connect to MongoDB
client = MongoClient("mongodb://localhost:27017/")
db = client["NewsScraper"]
collection = db["positive_news"]  # Save filtered news into a new collection
...
def find_news(source_name, source_config):
    ...
    with open("news_articles/news_article.txt", "a", encoding="utf-8") as f:
        ...
        for index, article in enumerate(articles[:10]):
            ...
            full_text = f"{headline}. {description_text}"

            if is_positive(full_text):
                f.write(f"\nHeadline: {headline}\n")
                f.write(f"Description: {description_text}\n")
                f.write(f"Source: {url}\n")
                f.write("=" * 50 + "\n")

                print(f"Saved POSITIVE article from {source_name}: {headline}")
                ...
                print(f"Skipped NON-positive article: {headline}")
